...
const maxForStudent = 2; // Change this varible to set how many groups a student can create!
...
knex.getGroups = userUid => {
  var groupArray = knex('group_membership').where("user_uid", userUid)
    ...
    .then(data => {
      ...
      throw new Error('error getting data from groups:' + err);
...
/**
  ...
  @params groupId = (int) groupId in the db
  @return (string) 'pairs deleted'
*/
...
knex.addPairs = (pairData, groupId) => {
  ...
  .then(genId => { // gets the generation Id for each pair
    ...
    return knex.batchInsert('pairs', rows, pairData.pairs.length) // insert all the pairs into database for history
      ...
      .catch(err => {
        throw new Error('Batch Inrest Failed due to: ' + err);
      }); // throw error if something went horribly wrong
  }).catch(err => {
    throw new Error('Unable to create generation, ' + err);
  }); // throw error if something went horribly wrong
};
...
.then(exist => {
  ...
  .then(next => {
    ...
    throw new Error('unable to create new generation,' + err);
  }); // throw error if something went horribly wrong
  } return exist[0].id; // if exist it will just return old generation id
}).catch(err => {
  throw new Error('parems aren\'t correct when calling addGeneration, ' + err);
}); // throw error if something went horribly wrong
}
...
knex.getPairsForGroup = groupId => {
  return knex('generations').where("group_id", groupId)
        ...
        .catch(err => {
          throw new Error('cannot get join pairs and gen table, ' + err);
        });  // throw error if something went horribly wrong
};
...
knex.getGenerationsByGroup = groupId => {
  ...
  .catch(err => {
    throw new Error('database off-line, '+ err);
  }); // throw error if something went horribly wrong
};
...
knex.getNewGen = groupId => {
  ...
  .then(next => {
    ...
    .catch(err => {
      throw new Error('cannot find pairs, ' + err);
    }); // throw error if something went horribly wrong
  }).catch(err => {
    throw new Error('cannot find gen_id, ' + err);
  }); // throw error if something went horribly wrong
};
